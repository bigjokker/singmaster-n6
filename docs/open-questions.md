...
**Correction 2026-08-23.** The paragraph above is the record as it stood on
2026-08-20 and is kept for the history; two of its claims are no longer true
of the tree.

* *`clean=True` in 7,508 s on 4 workers.* `results/i8_sweep.json` was
  regenerated by the corrected pipeline (`52dc1ac`, 2026-08-22) and now reads
  `clean=false`, `certificate=null`, `n_z_alive=1`,
  `z_survivors=[{k: 1021, ...}]`, `seconds=174.308`, `workers=8`. The old
  record said the run found nothing alive, which was false: it had *dropped*
  k=1021 rather than killing it. The 174 s figure is that regeneration, not
  a from-scratch cost. The original record is not lost -- it is in git, at
  `2e12d5f` and `4a2b1e2` (`seconds: 7508.037`, `workers: 4`,
  `clean: true`); it is the *working tree* that moved on.
* *One uniform pass, every column.* Two of the 5,182,634 rows are not from the
  sweep: k=2 is an engine fill (`n_filled_from_engine: 1`) and k=1021 was
  repaired 3517 → 1051 (`repaired` in the npz meta, `73f1ec9`).

`N(m_8) = 6` is unaffected -- it rests on the witness table, which verifies,
and on the coverage ledger. What changed is where the closure is recorded:
the sweep json is an honest dirty record, and since 2026-08-23 the ledger
reports i=8 as *coverage complete, UNBOUND* (complete coverage, no
certificate naming the table) rather than COMPLETE.
...
**Correction 2026-08-23.** The premise "no column has reached a cap" was true
when this was declined and is not true now. `results/i9_sweep.json` records
four columns alive at Z-jump round 12 of 12 -- k=87, 399, 553, 1281 -- which
is why that run is `clean=false`; all four were then killed by full Lucas at
p < sqrt(N) (191/421/557/1321) and carry engine witnesses in the table. i=8's
own Z-jump also ran to 14 rounds, not the 7/12 quoted above. The decline
still stands on its merits -- reaching a cap is not by itself an anomaly, and
the four were ordinary by the size law -- but the argument should not lean on
"never occurred". The escalation gap that *was* real is fixed separately
(D4, 2026-08-23): the per-round rule could not judge a lone column that
reaches the cap, so `RoundLedger.cap_survivors` now judges each one by
peers x Lambda over the primes it actually faced.
...
**Correction 2026-08-23.** [`q11-what-is-the-claim.md`](q11-what-is-the-claim.md)
was written while i=9 was still running and says so; i=9 finished
2026-08-21 and its record is in the tree. Two of its statements need
reading with that date in mind: the re-open condition "(a) i=9's escalation
trigger fires" points at a ledger `results/i9_sweep.json` never got (it is a
resumed-leg record with no `escalation` block and no Band II phases), and
its column counts in the cost table are one too high per row -- the count
is `k_max - 3`, i.e. 5,182,634 at i=8 and 35,522,326 at i=9, matching the
witness tables and the ledger.

What the gate asked for was recomputed during the 2026-08-23 audit and is
**ordinary** on every round. Band II regenerates from the model in seconds
(`sizelaw.expected_alive` over i=9's Band II window): expected
702,981 / 86,354 / 12,432 / 1,986 / 332 / 57.1 / 8.52 / 1.11 / 0.204 for
passes 1..9 against the recorded 703,000 / 86,212 / 12,411 / 1,946 / 318 /
47 / 6 / 1 / 0. The Z-jump rounds had to be recomputed from the 219 MB
checkpoint (do not re-walk it): rounds 3..12 expected
2,866 / 476 / 130 / 59 / 33.8 / 19.8 / 11.0 / 6.4 / 4.08 / 4.08 against the
recorded 2,842 / 437 / 140 / 66 / 35 / 19 / 11 / 7 / 7 / 4. No round
escalates, and the four cap survivors are ordinary under the D4 check as
well. So the answer to the gate is "it did not fire", but that is recorded
here rather than in the run's json.
...
**Closed 2026-08-23.** The quartic is an elliptic curve over Q with Jacobian
`E: Y^2 = X^3 - 792X + 9801`, `E(Q) = Z/2 x Z^2` -- rank EXACTLY 2 by descent
via the 2-isogeny with kernel `(-33,0)`. Magma `IntegralQuarticPoints` on
`[5,1320,126456,5102240,72824400]` with seed `[-250,74880]` returned exactly
the fifteen `a`-values of the `|a|<=10^7` search (largest 730); `Rank(E)`
printed `2 true`; `Saturation` of the MW generators is the same group
(index 1). No extra `a`, so no extra `c` with `10! | c`. Chebotarev pass
(2026-08-23, `k10_chebotarev.py`):
...
b <= 10^12 find nothing on it beyond c = 0. The remaining Chebotarev statement was "the no-kill locus has no integer
point with c != 0" -- now implied by Magma's complete list. Audited with
Q27's battery-depth lesson (2026-08-23, `k10_deep.py`): the locus SURVIVES
every deep p-adic battery, so congruences do not close it; Magma was
required and was run. Pinned by `scripts/test_k10_intersective.py`.
...
## Tier A3 -- from the 2026-08-22 witness-builder audit

### Q30. Make chunk coverage exact by writing `ks` on sparse records. *(difficulty: low - **HARD**)* -- DONE 2026-08-23

**Done as specified.** `family_sweep._job` writes `ks` on a chunk record
exactly when `n_cols != k_hi - k_lo + 1`; `witness._spans` returns
(spans, exact columns) and a record carrying `ks` contributes its columns
rather than its span, so `_scanned` infers nothing on a checkpoint written
after this. A `ks` list that contradicts its own record (wrong length, or a
column outside `[k_lo, k_hi]`) is refused rather than used. The key is
optional, so `CHECKPOINT_SCHEMA` stays at 2 and nothing existing is
rewritten -- **existing checkpoints keep the over-covering span, by
design**; only runs made after this get exact coverage.

Verified on the real checkpoints: i=3..7 rebuild sha-identical to the
shipped tables, i=8 rebuilds with 0 primes differing and k=1021 still left
unresolved for the engine. Worth being precise about which guard does that
on a pre-Q30 file: round 13 is a single record with `k_lo = k_hi = 145`,
`n_cols = 1` -- *dense*, so its span is exact and simply does not contain
1021. Coverage (`_spans` / `_scanned`) is what hands that column to the
engine; the count identity passes (1 == 1) and is the fallback for a column
hidden inside a *sparse* span, which is the case this question closes. The sparse-span-inside case is pinned in both directions
in `test_witness.py` -- a legacy record must still RAISE, the same record
with `ks` must leave the column unresolved for the engine and not raise --
for the Z-jump and Band II halves alike, and `test_closure.py` pins
end-to-end that every sparse record a real sweep writes carries a correct
list (11 sparse / 884 dense at i=5).

The measured table below is the estimate this was budgeted against; it is
kept as written.

**Why it exists.** The builder no longer re-derives which columns a round
tested; it reads coverage off the records (`b6fa90c`). A record is exact when
`n_cols == k_hi - k_lo + 1`, because a contiguous chunk covers its whole span.
When the round's live columns are scattered, `[k_lo, k_hi]` OVER-covers, and a
never-tested column falling inside that span is indistinguishable from a killed
one. `_spans` over-covers deliberately -- the safe direction, since a genuinely
scanned column always lies inside its own record's span -- so such a column is
caught by the count identity and RAISES instead of being repaired.

**Why it is not negligible.** Deferrals happen in LATE rounds, and late rounds
are exactly the sparse ones. i=8's k=1021 escaped the sparse case only because
round 13 happened to be dense and did not contain it; one surviving column above
k=1021 in that round and the span swallows it, turning the automatic engine
repair into a failed build.

**Measured 2026-08-22:**

| | i=7 | i=8 |
|---|---|---|
| records | 28,348 | 166,044 |
| dense (already exact) | 99.5% | 99.8% |
| sparse records | 138 | 377 |
| sparse columns | 26,617 | 180,362 |
| cost of explicit `ks` on sparse only | 0.21 MB | 1.44 MB |

About 4% of a 36.5 MB checkpoint, and nothing at all for the dense 99.8%.

**The work.** Write `ks` on a chunk record only when
`n_cols != k_hi - k_lo + 1`; have `_spans` / `_scanned` in `scripts/witness.py`
prefer an explicit `ks` when present. Coverage then becomes exact everywhere,
the builder infers nothing, and both remaining guards -- `declared_alive` and
the count identity -- drop to redundancy checks that should never fire.

**Timing matters.** This changes what the sweep WRITES, so it only helps runs
made after it. Existing checkpoints keep the over-covering span. Worth doing
before i=10 rather than discovering it during a multi-day run.
